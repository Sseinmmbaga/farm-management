            ...
            <div class="card">
                <div class="card-header bg-primary text-white">
                    <div class="d-flex justify-content-between align-items-center">
                        <h4 class="mb-0">
                            <i class="fas fa-calendar-plus me-2"></i> Schedule Farm Visit
                        </h4>
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('visits.store') }}" enctype="multipart/form-data">
                            ...
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    ...
                                    <input type="text" 
                                           ...
                                           id="visit_number" 
                                           name="visit_number" 
                                           value="{{ old('visit_number', $visitNumber ?? '') }}" 
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select @error('purpose') is-invalid @enderror" 
                                        ...
                                        <option value="inspection" {{ old('purpose') == 'inspection' ? 'selected' : '' }}>Inspection</option>
                                        ...
                                        <option value="monitoring" {{ old('purpose') == 'monitoring' ? 'selected' : '' }}>Monitoring</option>
                        ...
                        <div class="form-section">
                            <h5 class="mb-3 border-bottom pb-2">
                                <i class="fas fa-sticky-note me-2"></i> Report & Photos (Can be added later)
                            </h5>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('report') is-invalid @enderror" 
                                          id="report" 
                                          name="report" 
                                          rows="4">{{ old('report') }}</textarea>
                                <small class="text-muted">Optional. Can be filled after visit completion.</small>
                                @error('report')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                <label class="form-check-label">
                                    <input type="checkbox" 
                                           ...
                                           name="has_photos" 
                                           value="1" 
                                           {{ old('has_photos') ? 'checked' : '' }}>
                                    Mark as having photos (if uploaded later)
                                </label>
                                @error('has_photos')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="d-flex justify-content-between mt-4">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save me-1"></i> Schedule Visit
                            </button>
                        </div>
                    </form>
                </div>
...
@push('scripts')
<script>
    // Auto-generate visit number if empty
    document.addEventListener('DOMContentLoaded', function() {
        const visitNumberField = document.getElementById('visit_number');
        if (visitNumberField && !visitNumberField.value) {
            // Example auto-generation; you could fetch from server via AJAX
            const now = new Date();
            const year = now.getFullYear();
            const month = String(now.getMonth() + 1).padStart(2, '0');
            const day = String(now.getDate()).padStart(2, '0');
            const random = Math.floor(Math.random() * 1000).toString().padStart(3, '0');
            visitNumberField.value = `V-${year}${month}${day}-${random}`;
        }

        // Dynamic cascading for location selects could be added here
    });
</script>
@endpush