...
@section('title', $recordType === 'new' ? 'New Farm Record (Form 2)' : 'Existing Farm Record (Form 3)')
...
<style>
    ...
    .form-section {
        border-left: 4px solid {{ $recordType === 'new' ? '#27ae60' : '#3498db' }};
        padding-left: 15px;
        margin-bottom: 30px;
    }

    .form-section h4 {
        color: {{ $recordType === 'new' ? '#27ae60' : '#3498db' }};
        margin-bottom: 15px;
    }
...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2>
            @if($recordType === 'new')
                <i class="fas fa-file-alt text-success"></i> New Farm Record (Form 2)
            @else
                <i class="fas fa-file-alt text-primary"></i> Existing Farm Record (Form 3)
            @endif
        </h2>
    ...
    @if($existingRecord)
        <div class="alert alert-warning">
            ...
            A {{ $recordType }} record already exists for this farm and season.
            <a href="{{ route('farm-records.edit', $existingRecord) }}" class="alert-link">Edit existing record</a>
        </div>
    @endif
    ...
    <div class="form-container">
        ...
        <form method="POST" action="{{ route('farm-records.store') }}" id="farmRecordForm">
            @csrf
                        ...
                        <select class="form-select" id="farm_id" name="farm_id" required>
                            ...
                            @foreach($farms as $farmOption)
                                <option value="{{ $farmOption->id }}"
                                        {{ (old('farm_id') ?? $farm?->id) == $farmOption->id ? 'selected' : '' }}>
                        ...
                        <select class="form-select" id="season_id" name="season_id" required>
                            ...
                            @foreach($seasons as $seasonOption)
                                <option value="{{ $seasonOption->id }}"
                                        {{ (old('season_id') ?? $season?->id) == $seasonOption->id ? 'selected' : '' }}>
                                    {{ $seasonOption->name }}
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" class="form-control" id="cattle_count" name="cattle_count"
                               value="{{ old('cattle_count', 0) }}" min="0" placeholder="0">
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" class="form-control" id="goats_sheep_count" name="goats_sheep_count"
                               value="{{ old('goats_sheep_count', 0) }}" min="0" placeholder="0">
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" class="form-control" id="oxen_count" name="oxen_count"
                               value="{{ old('oxen_count', 0) }}" min="0" placeholder="0">
                    </div>
                </div>
            </div>
                ...
                <div class="checkbox-group">
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="has_chemical_seed_residue"
                               ...
                               {{ old('has_chemical_seed_residue') ? 'checked' : '' }}>
                        <label class="form-check-label" for="has_chemical_seed_residue">
                            <i class="fas fa-seedling text-warning"></i> Has Chemical Seed Residue (Mabaki ya Mbegu za Kemikali)
                        </label>
                    </div>
                    ...
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="has_chemical_residue"
                               ...
                               {{ old('has_chemical_residue') ? 'checked' : '' }}>
                        <label class="form-check-label" for="has_chemical_residue">
                            <i class="fas fa-exclamation-triangle text-danger"></i> Has Chemical Residue (Mabaki ya Kemikali)
                        </label>
                    </div>
                </div>
            </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" step="0.01" class="form-control" id="area_size" name="area_size"
                               value="{{ old('area_size') }}" min="0" placeholder="e.g., 2.5">
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" class="form-control" id="registration_year" name="registration_year"
                               value="{{ old('registration_year', date('Y')) }}"
            ...
            <!-- Land Changes (Only for Existing Farm Records - Form 3) -->
            <div class="form-section">
                ...
                <div class="land-change-section">
                    <p class="text-muted mb-3">
                        ...
                        Record any land area changes since the last season.
                    </p>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control" id="land_bought" name="land_bought"
                                   value="{{ old('land_bought') }}" min="0" placeholder="0.00">
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control" id="land_sold" name="land_sold"
                                   value="{{ old('land_sold') }}" min="0" placeholder="0.00">
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control" id="land_borrowed" name="land_borrowed"
                                   value="{{ old('land_borrowed') }}" min="0" placeholder="0.00">
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control" id="land_lent" name="land_lent"
                                   value="{{ old('land_lent') }}" min="0" placeholder="0.00">
                        </div>
                    </div>
            ...
            <div class="form-section">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select" id="impact_type" name="impact_type">
                            ...
                            <option value="positive" {{ old('impact_type') == 'positive' ? 'selected' : '' }}>Positive</option>
                            <option value="neutral" {{ old('impact_type') == 'neutral' ? 'selected' : '' }}>Neutral</option>
                            <option value="negative" {{ old('impact_type') == 'negative' ? 'selected' : '' }}>Negative</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="notes" name="notes" rows="3"
                              placeholder="Any additional notes about this record...">{{ old('notes') }}</textarea>
                </div>
            </div>

            <div class="d-flex justify-content-between mt-4">
                <a href="{{ route('farm-records.index') }}" class="btn btn-secondary">
                    <i class="fas fa-arrow-left"></i> Cancel
                </a>
                <button type="submit" class="btn btn-{{ $recordType === 'new' ? 'success' : 'primary' }}">
