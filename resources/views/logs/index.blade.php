...
<style>
    ...
    .log-card.type-seeding { border-left-color: #28a745; }
    .log-card.type-input { border-left-color: #17a2b8; }
    .log-card.type-observation { border-left-color: #ffc107; }
    .log-card.type-harvest { border-left-color: #fd7e14; }
    .log-card.type-activity { border-left-color: #6c757d; }
    .log-card.type-training { border-left-color: #6f42c1; }
    .log-card.type-inspection { border-left-color: #e83e8c; }
    ...
    .log-type-badge.type-seeding { background: #d4edda; color: #155724; }
    .log-type-badge.type-input { background: #d1ecf1; color: #0c5460; }
    .log-type-badge.type-observation { background: #fff3cd; color: #856404; }
    .log-type-badge.type-harvest { background: #ffe5d0; color: #a84300; }
    .log-type-badge.type-activity { background: #e2e3e5; color: #383d41; }
    .log-type-badge.type-training { background: #e2d9f3; color: #432874; }
    .log-type-badge.type-inspection { background: #f8d7da; color: #721c24; }
...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">Track all farm activities and observations</p>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('logs.quick-entry') }}" class="btn btn-outline-primary">
    ...
    <!-- Quick Type Filters -->
    <div class="mb-3">
        <a href="{{ route('logs.index') }}" class="type-filter-btn btn {{ !request('type') ? 'active' : '' }}">
            <i class="bi bi-grid"></i> All
        </a>
        @foreach($logTypes as $logType)
        <a href="{{ route('logs.index', ['type' => $logType->value]) }}"
           class="type-filter-btn btn {{ request('type') == $logType->value ? 'active' : '' }}">
    ...
    @if($logs->count() > 0)
        @foreach($logs as $log)
        <div class="log-card type-{{ $log->type->value }}">
            <div class="d-flex justify-content-between align-items-start">
                <div class="flex-grow-1">
                    <div class="d-flex align-items-center gap-2 mb-2">
                        <span class="log-type-badge type-{{ $log->type->value }}">
    ...
    @else
        <div class="text-center py-5">
            ...
            <p class="text-muted">Start by creating your first activity log</p>
            <a href="{{ route('logs.create') }}" class="btn btn-primary">
                <i class="bi bi-plus-lg"></i> Create Log
            </a>
        </div>
    @endif
@endsection
