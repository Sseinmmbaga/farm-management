...
@section('title', 'Observation Logs')
...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-0">Observation Logs</h1>
            <p class="text-muted mb-0">Track all farm observations (pests, diseases, growth stages, etc.)</p>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('logs.quick-entry') }}?type=observation" class="btn btn-outline-primary">
                <i class="bi bi-lightning"></i> Quick Entry
            </a>
            <a href="{{ route('logs.create') }}?type=observation" class="btn btn-primary">
                <i class="bi bi-plus-lg"></i> New Observation Log
            </a>
        </div>
    </div>

    <!-- Quick Type Filters (Only observation active) -->
    <div class="mb-3">
        <a href="{{ route('logs.index') }}" class="type-filter-btn btn">
            <i class="bi bi-grid"></i> All Logs
        </a>
        @foreach($logTypes as $logType)
            @if($logType->value === 'observation')
    ...
    <div class="filter-section">
        <form method="GET" action="{{ route('logs.index') }}" class="row g-3">
            ...
            <div class="col-md-3">
                ...
                <input type="text" name="search" class="form-control" placeholder="Search observation logs..." value="{{ request('search') }}">
            </div>
            <div class="col-md-2">
            ...
            <div class="d-flex justify-content-between align-items-start">
                <div class="flex-grow-1">
                    <div class="d-flex align-items-center gap-2 mb-2">
                        <span class="log-type-badge type-observation">
    ...
    @else
        <div class="text-center py-5">
            <i class="bi bi-binoculars display-1 text-muted"></i>
            <h4 class="mt-3">No Observation Logs Found</h4>
            <p class="text-muted">Start by creating your first observation log</p>
            <a href="{{ route('logs.create') }}?type=observation" class="btn btn-primary">
                <i class="bi bi-plus-lg"></i> Create Observation Log
            </a>
        </div>
    @endif
@endsection