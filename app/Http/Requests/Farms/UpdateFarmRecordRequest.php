...
class UpdateFarmRecordRequest extends FormRequest
{
    ...
    {
        $farmRecordId = $this->route('farmRecord')->id;
    ...
    private function validateFarmerAccess($validator): void
    {
        ...
        if ($user->hasRole(UserRole::EXTENSION_OFFICER)) {
            ...
            if (!in_array($farm->farmer_id, $assignedFarmerIds)) {
                $validator->errors()->add('farm_id', 'You can only update records for farmers assigned to you.');
            }
            return;
        }
        ...
        if ($user->hasRole(UserRole::FARMER)) {
            if ($user->farmer->id != $farm->farmer_id) {
                $validator->errors()->add('farm_id', 'You can only update records for your own farms.');
            }
        }
    }
}
